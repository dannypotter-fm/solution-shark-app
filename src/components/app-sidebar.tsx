      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: FileText,
            variant: "ghost",
            href: "/dashboard/solutions?stage=draft",
          },
          {
            title: "In Review",
            icon: FileText,
            variant: "ghost",
            href: "/dashboard/solutions?stage=review",
          },
        ],
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Inbox,
            variant: "ghost",
            href: "/dashboard/approvals/rejected",
          },
        ],
      },
    ],
    ...
    <div className={cn("flex w-full flex-col gap-2", className)} {...props}>
      {items.map((item, index) => (
        <div key={index} className="flex flex-col gap-2">
          <h4 className="px-2 text-xs font-medium leading-none tracking-tight text-muted-foreground">
            {item.title}
          </h4>
          {item.items?.length &&
            item.items.map((subItem, index) => (
...
export function AppSidebar({ className, ...props }: AppSidebarProps) {
  ...
  return (
    <Sidebar className={cn("pb-12", className)} collapsible="icon" {...props}>
      <AppSidebarContent />
    </Sidebar>
  )
}
    ...
    <>
      <SidebarHeader className="border-b px-4 py-4">
        <Link href="/dashboard" className="flex items-center gap-3 w-full">
          <div className="flex items-center justify-center w-8 h-8 border border-gray-300 rounded-md">
            <svg className="size-5" viewBox="0 0 1024 1024" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-sm">SolutionShark</span>
        ...
        <SidebarMenu>
          {items.map((group, groupIndex) => (
            <SidebarGroup key={groupIndex}>
              <SidebarGroupLabel className="px-3 text-xs font-medium leading-none tracking-tight text-muted-foreground">
                {group.title}
              </SidebarGroupLabel>
              <SidebarGroupContent>
